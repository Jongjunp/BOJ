...
int main() {
    ...
    //BFS
    queue< pair < pair< pair<int,int>,pair<int,int> >, queue<char> > > state_queue;
    pair< pair<int,int>,pair<int,int> > init_coords = make_pair(ori_red_coor,ori_blue_coor);
    queue<char> empty;
    pair < pair< pair<int,int>,pair<int,int> >,queue<char> > init_state = make_pair(init_coords, empty);
    int min_action_num = INF;
    queue<char> current_action_queue;
    state_queue.push(init_state);
    while (!state_queue.empty()) {
        pair < pair< pair<int,int>,pair<int,int> >, queue<char> > state = state_queue.front();
        pair< pair<int,int>,pair<int,int> > coords = state.first;
        current_action_queue = state.second;
        state_queue.pop();
        if (current_action_queue.size() > 10) {
            continue;
        }
        ...
        if (!((left_state.first.first==red_h)
            ...
            if ((left_state.first.first==goal.first) && (left_state.first.second==goal.second)) {
                if (!((left_state.second.first==goal.first) && (left_state.second.second==goal.second))
                    && (current_action_queue.size()+1 < min_action_num)) {
                    min_action_num = current_action_queue.size()+1;
                    current_action_queue.push('L');
                    break;
                }
            }
            else {
                if (!((left_state.second.first==goal.first) && (left_state.second.second==goal.second))) {
                    queue<char> left_action_queue(current_action_queue);
                    left_action_queue.push('L');
                    pair < pair< pair<int,int>,pair<int,int> >,queue<char> > new_state = make_pair(left_state,left_action_queue);
        ...
        if (!((right_state.first.first==red_h)
            ...
            if ((right_state.first.first==goal.first) && (right_state.first.second==goal.second)) {
                if (!((right_state.second.first==goal.first) && (right_state.second.second==goal.second))
                    && (current_action_queue.size()+1 < min_action_num)) {
                    min_action_num = current_action_queue.size()+1;
                    current_action_queue.push('R');
                    break;
                }
            }
            else {
                if (!((right_state.second.first==goal.first) && (right_state.second.second==goal.second))) {
                    queue<char> right_action_queue(current_action_queue);
                    right_action_queue.push('R');
                    pair < pair< pair<int,int>,pair<int,int> >,queue<char> > new_state = make_pair(right_state,right_action_queue);
                    state_queue.push(new_state);
                }
            }
        }
        ...
        if (!((down_state.first.first==red_h)
            ...
            if ((down_state.first.first==goal.first) && (down_state.first.second==goal.second)) {
                if (!((down_state.second.first==goal.first) && (down_state.second.second==goal.second))
                    && (current_action_queue.size()+1 < min_action_num)) {
                    min_action_num = current_action_queue.size()+1;
                    current_action_queue.push('D');
                    break;
                }
            }
            else {
                if (!((down_state.second.first==goal.first) && (down_state.second.second==goal.second))) {
                    queue<char> down_action_queue(current_action_queue);
                    down_action_queue.push('D');
                    pair < pair< pair<int,int>,pair<int,int> >,queue<char> > new_state = make_pair(down_state,down_action_queue);
                    state_queue.push(new_state);
                }
            }
        }
        ...
        if (!((up_state.first.first==red_h)
            ...
            if ((up_state.first.first==goal.first) && (up_state.first.second==goal.second)) {
                if (!((up_state.second.first==goal.first) && (up_state.second.second==goal.second))
                    && (current_action_queue.size()+1 < min_action_num)) {
                    min_action_num = current_action_queue.size()+1;
                    current_action_queue.push('U');
                    break;
                }
            }
            else {
                if (!((up_state.second.first==goal.first) && (up_state.second.second==goal.second))) {
                    queue<char> up_action_queue(current_action_queue);
                    up_action_queue.push('U');
                    pair < pair< pair<int,int>,pair<int,int> >,queue<char> > new_state = make_pair(up_state,up_action_queue);
                    state_queue.push(new_state);
                }
            }
        }
    ...
    if (min_action_num > 10) {
        ...
        while (!current_action_queue.empty()) {
            cout << current_action_queue.front();
            current_action_queue.pop();
        }
        cout << endl;
    }
}