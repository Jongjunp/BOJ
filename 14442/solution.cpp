...
int main() {
    int height, width, breaknum;
    char board[1000][1000] = {'0'};
    cin >> height >> width >> breaknum;
    ...
    while (!state_queue.empty()) {
        ...
        //left action
        if (w!=0) {
            if ((board[h][w-1]==blank) && (!visited[h][w-1][breaked])) {
        ...
        if (w!=width-1) {
            if ((h==height-1) && (w+1==width-1)) {
                ...
                if ((breaked < breaknum) && (board[h][w+1]==block)) {
        ...
        //up action
        if (h!=0) {
            if ((board[h-1][w]==blank) && (!visited[h-1][w][breaked])) {
        ...
        if (h!=height-1) {
            if ((h+1==height-1) && (w==width-1)) {
                ...
                if ((breaked < breaknum) && (board[h+1][w]==block)) {
                        cout << breaked << endl;
